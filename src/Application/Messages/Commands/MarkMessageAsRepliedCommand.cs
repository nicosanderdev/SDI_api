...
public record MarkMessageAsRepliedCommand(Guid MessageId, Guid UserId) : IRequest;

public class MarkMessageAsRepliedCommandHandler : IRequestHandler<MarkMessageAsRepliedCommand>
{
    private readonly IApplicationDbContext _context;

    public MarkMessageAsRepliedCommandHandler(IApplicationDbContext context)
    {
        _context = context;
    }

    public async Task Handle(MarkMessageAsRepliedCommand request, CancellationToken cancellationToken)
    {
        ...
        if (!recipient.HasBeenRepliedToByRecipient)
        {
            recipient.HasBeenRepliedToByRecipient = true;
