...
public class GetDashboardSummaryQueryHandler : IRequestHandler<GetDashboardSummaryQuery, DashboardSummaryDataDto>
{
    ...
    private async Task<long> GetCountAsync(IQueryable<dynamic> queryableSource, DateTime startDate, DateTime endDate, CancellationToken cancellationToken)
    {
        ...
        if (queryableSource is IQueryable<EstateProperty> properties) // For total properties
        {
            ...
            return await properties.LongCountAsync(p => p.IsPublic, cancellationToken);
        }
        return 0;
    }
    ...
    public async Task<DashboardSummaryDataDto> Handle(GetDashboardSummaryQuery request, CancellationToken cancellationToken)
    {
        ...
        long totalActiveProps = await _context.EstateProperties.LongCountAsync(p => p.IsPublic, cancellationToken);
